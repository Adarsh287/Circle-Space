...
async function loadPosts() {
  try {
    ...
    if (currentFilter === "popular") {
      query = db.collection("posts").orderBy("likes", "desc")
    } else if (currentFilter === "commented") {
      query = db.collection("posts").orderBy("commentCount", "desc")
    }
    ...
    if (snapshot.empty && !lastVisible) {
      postsContainer.innerHTML = '<p class="no-posts">No posts found. Be the first to create a post!</p>'
